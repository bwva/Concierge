...
sub build_desk ($storage_dir, $auth_file, $app_fields=[], $user_session_fields=[]) {
	...
	warn "DEBUG: Sessions object created, storage_dir=$storage_dir\n" if $ENV{DEBUG_CONCIERGE};
	my $session_result	= $self->{sessions}->new_session(
		user_id         => '__concierge__',
		session_timeout => 'indefinite',
		admin_session   => 1,
	);
	warn "DEBUG: new_session result: success=" . $session_result->{success} .
	      ($session_result->{success} ? "" : ", message=" . $session_result->{message}) . "\n"
	      if $ENV{DEBUG_CONCIERGE};
